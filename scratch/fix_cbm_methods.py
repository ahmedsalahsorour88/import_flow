import os

with open('frontend/lib/features/cbm_calculator/widgets/saved_cbm_registry_tab.dart', 'r', encoding='utf-8') as f:
    tab_lines = f.readlines()

save_start = -1
save_end = -1
for i, l in enumerate(tab_lines):
    if 'void _showSaveCalcDialog(' in l:
        save_start = i
    if 'void _showCalcDetailsDialog(' in l:
        save_end = i
        break

print(f"save_start: {save_start}, save_end: {save_end}")

save_methods = "".join(tab_lines[save_start:save_end])
clean_tab_lines = tab_lines[:save_start] + tab_lines[save_end:]

with open('frontend/lib/features/cbm_calculator/widgets/saved_cbm_registry_tab.dart', 'w', encoding='utf-8') as f:
    f.writelines(clean_tab_lines)

print("Cleaned saved_cbm_registry_tab.dart!")

with open('frontend/lib/features/cbm_calculator/screens/cbm_calculator_screen.dart', 'r', encoding='utf-8') as f:
    screen_code = f.read()

idx = screen_code.find('  Widget _buildSavedRegistryTab(')
screen_code = screen_code[:idx] + save_methods + "\n" + screen_code[idx:]

with open('frontend/lib/features/cbm_calculator/screens/cbm_calculator_screen.dart', 'w', encoding='utf-8') as f:
    f.write(screen_code)

print("Updated cbm_calculator_screen.dart with save dialog!")
