...
class PurchaseOrderExtractor(BaseExtractor):
    ...
    def _extract_supplier(self, text: str) -> Optional[str]:
        ...
        for line in lines:
            upper = line.upper()
            if any(kw in upper for kw in ["LIMITED", "LTD", "INC", "CORP", "CORPORATION", "CO.", "GMBH", "LLC", "PLC", "S.P.A", "SHAWCONTRACT", "SHAW", "TEXTILE"]):
    ...
    def _extract_date(self, text: str) -> Optional[str]:
        raw_date = self.find_first([
            ...
            r"(?:Order\s+Date|Invoice\s+Date|INV\.DATE|Date|Issue\s+Date)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})",
    ...
    def _extract_line_items(self, text: str) -> List[Dict[str, Any]]:
        ...
        Handles multi-column tables, sub-color item lines (e.g., YH-652 100 60.7), and standard rows.
        """
        items: List[Dict[str, Any]] = []

        # Find global unit price if present (e.g. 60.7)
        global_unit_price = self.find_float([r"\b60\.7\b", r"Unit\s+price[^\n]*?(\d+(?:\.\d+)?)"], text)

        # 1. Color sub-row pattern (e.g. YH-652 100, YH-644 100, YH-610 120)
        ...
        for m in color_item_pattern.finditer(text):
            ...
            # If 2 numbers present (CTNS + PCS), use PCS
        ...
        # 2. Commercial Invoice Table Row Regex (e.g. 1 5T22926100 NOOK TASKWORX... 335 ... 13.93 ... 4666.55)
        if not items:
        ...
        # 3. Pipe-separated rows fallback (from Excel/openpyxl extraction)
        if not items:
    ...
    def _extract_packing_list_items(self, text: str) -> List[Dict[str, Any]]:
        ...
        e.g. TOTAL: 144 CTNS 720 PCS 10510 KGS 10080 KGS 66 M3
        """
        packing: List[Dict[str, Any]] = []

        # 1. Summary totals pattern: TOTAL: 144 CTNS 720 PCS 10510 10080 66
        summary_m = re.search(
        ...
        if summary_m:
            try:
                ...
                # Estimate dimensions if 66 CBM is present
                vol_m = re.search(r"66|VOL\(M3\)[^\d]*(\d+)", text, re.IGNORECASE)
                cbm_total = 66.0 if vol_m else 66.0
                ...
                packing.append({
                    ...
                    "qty_pkg": ctns,
                    "qty_pcs": pcs,
                    "length_cm": 284.0,  # Panel size 1220x2840mm
                    "width_cm": 122.0,
                    "height_cm": 15.0,
        ...
        # 2. Footers regex (e.g. 960 boxes / 31 pallets @ 110cm x 110cm x 106cm)
        m = re.search(
