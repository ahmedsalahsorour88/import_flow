...
def write_screen():
    target_path = r"frontend/lib/features/shipping_scenarios/screens/shipping_scenarios_screen.dart"
...
class _ShippingScenariosScreenState extends ConsumerState<ShippingScenariosScreen>
  ...
  double _calculateTotalQuoteValue(ShippingScenarioItemModel item, List<CurrencyModel> currencies) {
    ...
    if (item.vgmNotificationApplicable) {
      final totalQty = item.container40ftQty + item.container20ftQty;
  ...
  void _updateItem(int idx, ShippingScenarioItemModel updatedItem, List<CurrencyModel> currencies) {
    final totalAmount = _calculateTotalQuoteValue(updatedItem, currencies);
    setState(() {
                                        ...
                                        TextButton.icon(
                                          ...
                                          label: Text(
                                            isExpanded 
                                                ...
                                                : 'تفاصيل عرض السعر (Edit Quote) [${item.totalQuotationAmount.toStringAsFixed(1)} ${item.quotationCurrency}]',
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Expanded(
                                          child: SearchableDropdownField<String>(
                                            value: item.quotationCurrency,
                                            labelText: 'Main Quote Currency (عملة المقارنة الأساسية للعرض)',
                                            items: const [
                                              SearchableDropdownItem(value: 'USD', label: r'USD - Dollar (\$)' ),
                                              SearchableDropdownItem(value: 'EUR', label: 'EUR - Euro (€)'),
                                              SearchableDropdownItem(value: 'EGP', label: 'EGP - Egyptian Pound (ج.م)'),
                                            ],
                                            onChanged: (v) {
                                              if (v != null) {
                                    ...
                                    Padding(
                                      ...
                                      child: Text(
                                        'إجمالي عدد الحاويات الشاحنة = ${item.container40ftQty + item.container20ftQty} حاوية (تفصيل: 40ft: ${item.container40ftQty} | 20ft: ${item.container20ftQty})',
                                    ...
                                    _buildCostRow(
                                      ...
                                      qty: (item.container40ftQty + item.container20ftQty).toDouble(),
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          const Text(
                                            '💰 إجمالي قيمة هذا العرض (إجمالي كل البنود المطبقة):',
                                            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppTheme.charcoal),
                                          ),
                                          Text(
                                            '${item.totalQuotationAmount.toStringAsFixed(2)} ${item.quotationCurrency}',
                              ...
                              rows: calculatedScenarios.map((c) {
                                ...
                                return DataRow(
                                  cells: [
                                    ...
                                    DataCell(Text('${item.totalQuotationAmount.toStringAsFixed(1)} ${item.quotationCurrency}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.red))),
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      fontWeight: FontWeight.bold, 
                      fontSize: 12,
                      color: applicable ? AppTheme.charcoal : Colors.grey.shade500
                    ),
          ...
          if (applicable) ...[
            Expanded(
              ...
              child: TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'سعر البند / الفئة', isDense: true),
                onChanged: (v) {
            ...
            Expanded(
              ...
              child: SearchableDropdownField<String>(
                ...
                labelText: 'العملة',
                items: const [
                  SearchableDropdownItem(value: 'USD', label: r'USD (\$)' ),
            ...
            if (showQty) ...[
              ...
              Expanded(
                flex: 1,
                child: TextFormField(
                  key: qtyReadOnly ? ValueKey('readonly_qty_$qty') : null,
                  ...
                  decoration: const InputDecoration(labelText: 'الكمية / العدد', isDense: true),
              ...
              Expanded(
                flex: 1,
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'الإجمالي', isDense: true),
                  child: Text(
                    '${(price * qty).toStringAsFixed(1)} $currency',
            ...
            const Expanded(
              flex: 4,
              child: Text(
                'Not Applicable (غير مطبق لهذه الرحلة / الخط)',
  ...
  void _showPrintReportDialog(BuildContext context, ShippingEvaluationModel sess) {
    ...
    for (var item in sess.items) {
      buffer.writeln('"${item.providerName}","${item.providerName}","${item.customsBrokerName ?? "-"}","${item.vesselName}","${item.voyageNumber ?? "-"}","${item.polName ?? "-"}","${item.podName ?? "-"}","${item.sailingDate}","${item.estimatedArrivalDate}",${item.freeTimeDays},${item.expectedLineDelayDays},${item.expectedTotalDaysToWarehouse},"${item.totalQuotationAmount} ${item.quotationCurrency}","${item.riskLevel}","${item.isRecommended ? "Recommended" : item.isExcludedFromAverage ? "Excluded" : "Normal"}"');
    }
...
"""
    ...
    print("Rewrote shipping_scenarios_screen.dart successfully!")
