...
- `Ctrl + D`: استنساخ فوري للصف أو البند النشط (Row-Level Clone) وإدراجه مباشرة أسفله مع تركيز حقل التعديل.
...
### د. زر الاستنساخ العام ومحرك التكرار المنضبط (Universal Clone Engine — UX-CLONE-011)
تسريع إدخال البيانات المتكررة بنمط هندسي موحد بمستويين:
1. **استنساخ السطر (Row-Level Clone):**
   - أيقونة (⧉) في نهاية كل صف واختصار `Ctrl + D`.
   - إدراج سطر جديد متطابق مباشرة أسفل السطر الأصلي.
   - تركيز المؤشر التلقائي (Auto-focus) على الحقل الأكثر احتمالاً للتغيير (مثل السعر أو الكمية).
   - استنساخ متعدد لمجموعة أسطر محددة دفعة واحدة.
2. **استنساخ الكيان الكامل (Entity-Level Clone):**
   - استنساخ ملفات الشحنات وقوائم الأسعار والدراسات عبر نافذة مراجعة وسيطة إجبارية.
   - نسخ المورد والأصناف وشروط الدفع كما هي.
   - تصفير إجباري لأرقام البوالص والفواتير والحاويات وتاريخ الوصول لمنع النسخ الشبحي.
   - إعادة الحالة تلقائياً إلى "مسودة (Draft)".
   - استبعاد المرفقات افتراضياً وجعلها خياراً منفصلاً غير محدد.
   - تتبع الأصل المرجعي عبر `cloned_from_id` ورابط "🔗 مستنسخ من".

### هـ. دعم الوضع الليلي (Dark Theme) والتباين العالي
